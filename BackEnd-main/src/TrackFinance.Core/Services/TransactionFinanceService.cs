...
public class TransactionFinanceService : ITransactionFinanceService
{
  ...
  public async Task<Result<List<TransactionDataDto>>> GetTransactionItemsByDateAsync(int userId, TransactionType transactionType, DateTime dateIni, DateTime dateEnd, CancellationToken cancellationToken = default)
  {
    var itemByDate = await _repository.ListAsync(new ItemsByDateSpec(userId, transactionType, dateIni, dateEnd));
    return GetListTransactionByDateRange(itemByDate, dateIni, dateEnd);
  }
  ...
  private static Result<List<TransactionDataDto>> GetListTransactionByDateRange(List<Transaction> items, DateTime dateIni, DateTime endDate)
  {
    var transactionsResult = new List<TransactionDataDto>();

    var transactions = items
        .Where(h => h.ExpenseDate >= dateIni && h.ExpenseDate <= endDate)
        .GroupBy(h => new { h.TransactionType, h.TransactionDescriptionType })
        .Select(g => new
        {
          TotalAmount = g.Sum(h => h.Amount),
          g.Key.TransactionDescriptionType,
    ...
    {
      return Result<List<TransactionDataDto>>.NotFound();
    }
    return transactionsResult;
  }
